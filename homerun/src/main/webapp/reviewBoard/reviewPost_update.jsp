...
<body>
	...
	<form action="reviewPost_updateAction.jsp" method="post">
		...
		<table class="table table-bordered"
			...
			<tr>
				<th bgcolor="#E1EEDD" width="200"
					style="height: 30px; text-align: center; line-height: 30px;">경기일
					경기팀</th>
				<td><%=gDao.getGame(rbDto.getgId()).getgDay()%> <%=gDao.getGame(rbDto.getgId()).getHome()%>
					vs <%=gDao.getGame(rbDto.getgId()).getAway()%></td>
			</tr>

			<tr>
				<th bgcolor="#E1EEDD" width="100"
					style="height: 30px; text-align: center; line-height: 30px;">제목</th>
				<td><input type="text" name="subject" class="form-control"
					required="required" style="width: 500px;"
					value="<%=rbDto.getRbSubject()%>"></td>
			</tr>
			<tr>
